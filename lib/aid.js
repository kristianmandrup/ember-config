...
module.exports = function(ctx) {
  return {
    ...
    generate: function(name, options) {
      var cmArgs = ['generate'];
      options = options || []
      cmArgs.concat(options);
      console.log(name, cmArgs);
      ctx.spawnCommand(name, cmArgs);
    },

    installBower: function(compilerName) {
      this.installBow(compilerName, compilerName);  
    },

    installBow: function(alias, compilerName) {
      compilerName = compilerName || 'ember-cli-'.concat(alias);
    ...
    installNpm: function(compilerName) {
      this.install(compilerName, compilerName);  
    },
    install: function(alias, compilerName) {
      compilerName = compilerName || 'ember-cli-'.concat(alias);
    ...
    uninstallNpm: function(compilerName) {
      this.uninstall(compilerName, compilerName);  
    },
    uninstall: function(alias, compilerName) {
      compilerName = compilerName || 'ember-cli-'.concat(alias);
